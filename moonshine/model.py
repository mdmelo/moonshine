...
# operations, making code cleaner and more intuitive.
...
class AudioPreprocessor(object):
    def __init__(self, dim):
        ...
        # gaussian - Gaussian Error Linear Unit
...
# paper "Attention is all you Need". If `query`, `key,` `value` are the
# same, then this is self-attention. Each timestep in `query` attends to the
...
#
# Model Details
# The Moonshine models are trained for the speech recognition task, capable of
# transcribing English speech audio into English text. Useful Sensors developed the
# models to support their business direction of developing real time speech transcription
# products based on low cost hardware. There are 2 models of different sizes and
# capabilities, summarized in the following table.
#
...
# Evaluated Use
#
# The primary intended users of these models are AI developers that want to deploy
# English speech recognition systems in platforms that are severely constrained in
# memory capacity and computational resources. We recognize that once models are
# released, it is impossible to restrict access to only “intended” uses or to draw
# reasonable guidelines around what is or is not safe use.
#
# The models are primarily trained and evaluated on English ASR task. They may exhibit
# additional capabilities, particularly if fine-tuned on certain tasks like voice activity
# detection, speaker classification, or speaker diarization but have not been robustly
# evaluated in these areas. We strongly recommend that users perform robust evaluations
# of the models in a particular context and domain before deploying them.
#
# In particular, we caution against using Moonshine models to transcribe recordings
# of individuals taken without their consent or purporting to use these models for any
# kind of subjective classification. We recommend against use in high-risk domains like
# decision-making contexts, where flaws in accuracy can lead to pronounced flaws in
# outcomes. The models are intended to transcribe English speech, use of the model
# for classification is not only not evaluated but also not appropriate, particularly
# to infer human attributes.
#
# Training Data
# The models are trained on 200,000 hours of audio and the corresponding transcripts
# collected from the internet, as well as datasets openly available and accessible on
# HuggingFace. The open datasets used are listed in the the accompanying paper.
